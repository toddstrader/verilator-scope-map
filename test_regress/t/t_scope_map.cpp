...
int main(int argc, char **argv, char **env) {
    ...
    const VerilatedScopeNameMap* scopeMap = Verilated::scopeNameMap();
    for (VerilatedScopeNameMap::const_iterator it = scopeMap->begin(); it != scopeMap->end(); it++) {
        printf("---------------------------------------------\n");
        printf("Scope = %s\n", it->first);
        it->second->scopeDump();
        VerilatedVarNameMap * varNameMap = it->second->varsp();
        if (varNameMap == NULL) {
            printf("Error: Bad varsp()\n");
            return -1;
        }
        ...
        for (VerilatedVarNameMap::iterator varIt = varNameMap->begin(); varIt != varNameMap->end(); varIt++) {
            ...
            if (varRight != 0) {
                printf("Error: Was expecting right range value = 0\n");
                return -1;
            }
            ...
            for (int i = 0; i < varBits / 8; i++) {
                printf("%02x ", varData[i]);

                vluint8_t expected = i % 0xff;
                if (varData[i] != expected) {
                    printf("Error: Data mismatch, got 0x%02x, expected 0x%02x\n", varData[i], expected);
                    return -1;
                }
            }
    ...
    for (VerilatedScopeNameMap::const_iterator it = scopeMap->begin(); it != scopeMap->end(); it++) {
        ...
        for (VerilatedVarNameMap::iterator varIt = varNameMap->begin(); varIt != varNameMap->end(); varIt++) {
            ...
            for (int i = 0; i < varBits / 8; i++) {
                ...
                if (varData[i] != expected) {
                    printf("Error: Data mismatch (%s), got 0x%02x, expected 0x%02x\n", varIt->first, varData[i], expected);
                    return -1;
                }
            }
